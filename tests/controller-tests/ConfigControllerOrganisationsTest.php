...
class ConfigControllerOrganisationsTest extends ConfigControllerTestCase
{
    ...
    public function createNewOrganisation()
    {
        ...
        $this->seeInDatabase('users', ['email' => $data['twilio_user']['email'], 'organisation_id' => $data['_id']]);
    ...
    /**
     *
     * create new organisation with a new operations user
     *
     * @test
     */
    public function createOrganisationWithAnOperationsUser()
    {
        $user = $this->createUser();
        $org = $this->orgset[0];
        $name = 'op1';
    ...
    /**
     *
     * update organisation adding a new operations user
     *
     * @test
     */
    public function updateOrganisationAddingAnOperationsUser()
    {
        $user = $this->firstUser();
        $org = $this->orgset[0];
        $name = 'op1';
    ...
    /**
     *
     * update organisation existing operations user
     *
     * @test
     */
    public function updateOrganisationOperationsUser()
    {
        $user = $this->createUser();
        $org = $this->orgset[0];
        $name = 'op1';
        ...
        $namex = 'op1x';
        $emailx = 'op1x@example.com';
        $passwordx = 'mstgpwd2';

        // Act - adding new org with a ops user
        $this->actingAs($user)->json('post', '/organisation', [
            'name' => $org['name'],
            'users' => [
                [
        ...
        // Act some more modifying the ops user
        $this->actingAs($user)->json('put', '/organisation/' . $data['_id'], [
            'name' => $data['name'],
            'users' => [
                [
                    '_id' => $userId,
    ...
    /**
     *
     * update organisation delete an operations user
     *
     * @test
     */
    public function updateOrganisationDeleteOperationsUser()
    {
        $user = $this->createUser();
        $org = $this->orgset[0];
        $name = 'op1';
        ...
        // Act some more submit organisation with no ops user list
        $this->actingAs($user)->json('put', '/organisation/' . $data['_id'], [
            'name' => $data['name']
        ]);
